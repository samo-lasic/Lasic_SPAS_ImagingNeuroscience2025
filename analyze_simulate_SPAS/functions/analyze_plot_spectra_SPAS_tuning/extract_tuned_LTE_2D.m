...
function out = extract_tuned_LTE_2D(f, PS, trPS, ODF, g, q, dt, N0, parameters)
% Provisional version of extract_tuned_LTE_3D, adapted for PTE tensors.
...
[v,l] = eig(real(squeeze(sum(PS))));

phi = linspace(0,2*pi,500);
u = v(:,2).*cos(phi)+ v(:,3).*sin(phi);
u = u';
...
Du = sum(2*PSu'.*repmat(Dw,1,size(PSu,1)))/D0;
...
out.tunedLTE.g = sqrt(2)*g*out.tunedLTE.vec';
out.tunedLTE.q = sqrt(2)*q*out.tunedLTE.vec';
...
out.optLTE.g = sqrt(2)*g*out.optLTE.vec';
out.optLTE.q = sqrt(2)*q*out.optLTE.vec';
