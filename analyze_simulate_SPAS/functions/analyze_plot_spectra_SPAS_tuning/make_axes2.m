function make_axes2(parameters, spectral_PAS, L, ms, order)
% Unlike make_axes1, this function does not require the "filteredB_shape" field.
...
if nargin > 1
    if parameters.showSPAS % spectral PAS (SPAS)
        ...
        if abs(spectral_PAS.FA - 1) < 1e-10
            v = spectral_PAS.V(:,1);
