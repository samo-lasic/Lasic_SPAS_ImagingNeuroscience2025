...
function Tensor = tensor_from_projections(g,DirData)
% get tensor from tensor projections (DirData)
% g(n,3) = direction, DirData(n,1), outputs [V, l, m, FA, Delta, Eta]
    
    N = [g(:,1).*g(:,1) g(:,2).*g(:,2) g(:,3).*g(:,3) 2*g(:,1).*g(:,2) 2*g(:,1).*g(:,3) 2*g(:,2).*g(:,3)];
    v = N\DirData';
